...
function colsd(x)
    ...
    for i in 1: size(x)[2]
        m = mean(x[:,i]);
        y[i] = sqrt(sum((x[:,i].- m).^2) / (length(x[:,i]) - 1));
    end
    return y
end
#--------------------------------------------------
...
89-79+1, 103-90+1, 124-104+1,127-125+1, 139-128+1, 159-140+1,
175-160+1, 187-176+1, 200-188+1, 222-201+1, 1];
d = length(section);
...
function g(x,y)
    ...
    t = 1;
    for i in 1:d
        if x[t:(t+section[i]-1)] == y[t:(t+section[i]-1)]
            f[i] = 0;
        else
            f[i] = 1;
        end
...
@CPUtime for t in 1:M # record the CPU time
    ...
    if length(Active)>0
        for i in 2:l
            for j in 1:(i-1)
                m_mat[Model[i],Model[j]] = 1* (psi_path[1][t+1,(d+Model[j])]+psi_path[1][t+1,(d+Model[i])])+ dot(g(X[Model[i],:],X[Model[j],:])[Active], psi_path[1][t+1, Active]);
    ...
    for k in 1: n
        ...
        if length(Active)>0
            for i in C
                a = a +  ((z_mat[i,k]+tau*log(n))^2 - (z_mat[i,k]- psi_path[1][t+1,(d+k)] - psi_path[1][t+1,(d+i)] - dot( g(X[i,:], X[k,:])[Active], psi_path[1][t+1, Active]))^2 )/(2*psi_mat[i,k]);
            end
            else
            for i in C
